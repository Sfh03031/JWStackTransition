//
//  JWStackTransitionAnimationSplitFromCenter.swift
//  Pods
//
...
public class JWStackTransitionAnimationSplitFromCenter: JWStackTransitionAnimationDelegate {
    ...
    func setUpAnimation(duration: TimeInterval, transitionContext: UIViewControllerContextTransitioning) {
        ...
        var layer = animatedDiamondPath(duration: duration, startCenter: start, endCenter: CGPoint.init(x: start.x, y: start.y - (diamondSize.height / 2)), size: diamondSize, screenBounds: fromVC.view.bounds, completion: completion)
        containerLayer.addSublayer(layer)
        
        start = CGPoint.init(x: fromVC.view.bounds.width / 2, y: (fromVC.view.bounds.height / 2) + (diamondSize.height / 2))
        layer = animatedDiamondPath(duration: duration, startCenter: start, endCenter: CGPoint.init(x: start.x, y: start.y + (diamondSize.height / 2)), size: diamondSize, screenBounds: fromVC.view.bounds, completion: nil)
        containerLayer.addSublayer(layer)
        
        start = CGPoint.init(x: (fromVC.view.bounds.width / 2) + (diamondSize.width / 2), y: fromVC.view.bounds.height / 2)
        layer = animatedDiamondPath(duration: duration, startCenter: start, endCenter: CGPoint.init(x: start.x + (diamondSize.width / 2), y: start.y), size: diamondSize, screenBounds: fromVC.view.bounds, completion: nil)
        containerLayer.addSublayer(layer)
